...
def createDataset(inputPath, outputPath, checkValid=True):
    ...
    data = os.listdir(os.path.join(inputPath, "test_image"))

    nSamples = len(data)
    for i, d in enumerate(data):
        imagePath = d.replace("\\", "/")
        imagePath = os.path.join(inputPath, "test_image", imagePath)
        with open(imagePath, 'rb') as f:
            imageBin = f.read()
        if checkValid:
            try:
        ...
        cache[imageKey] = imageBin
        cache[labelKey] = "null character".encode()
        cache[pathkey] = ("test_image\\" + d).encode()
