...
def main():
    ...
    parser.add_argument('--test_speed', action='store_true', default=False, help='whethe to test speed')
    ...
    if "Union14M" not in args.data_root: 
        test_set = SceneTextDataModule.TEST_BENCHMARK_SUB + SceneTextDataModule.TEST_BENCHMARK + ("WordArt_train", "WordArt_test")
        if args.new:
        ...
        result_groups = {
            ...
            'WordArt': ("WordArt_train", "WordArt_test"),
        }
        if args.new:
            result_groups.update({'New': SceneTextDataModule.TEST_NEW})
    else:
        test_set1 = ("curve", "multi_oriented","artistic", "contextless", "salient" , "multi_words","general")
        test_set = test_set1 + ("incomplete", )
        result_groups = {
            'Union14M-L': test_set1,
    ...
    for name, dataloader in datamodule.test_dataloaders(test_set).items():
        ...
        confidence = 0
        label_length = 0
        if args.test_speed and 'WordArt_train' not in name:
            continue
        ...
        if args.test_speed and 'WordArt_train' in name:
            print(f"rnuning time: {time2 - time1}, samples have {total}, average fps {(time2-time1)/total*1000} ms/img.")
            exit()
    if "Union14M" in args.data_root:
        args.checkpoint = args.checkpoint + "_union"
    with open(os.path.join("test_results", args.checkpoint + '.txt'), 'w') as f:
