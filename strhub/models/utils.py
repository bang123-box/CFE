...
def _get_model_class(key):
    if 'abinet' in key:
        ...
        from .cfe.system import CFE as ModelClass
    else:
...
def get_pretrained_weights(experiment):
    if os.path.exists(os.path.join("checkpoint", experiment + ".pth")):
        return torch.load(os.path.join("checkpoint", experiment + ".pth"), map_location="cpu")
    try:
...
def create_model(experiment: str, pretrained: bool = False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(get_pretrained_weights(experiment), strict=False)
    return model
...
def load_from_checkpoint(checkpoint_path: str, **kwargs):
    if checkpoint_path.startswith('pretrained='):
        ...
        checkpoint_path = "checkpoint/" + checkpoint_path + ".ckpt"
