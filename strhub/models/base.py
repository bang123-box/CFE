...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation: bool) -> Optional[STEP_OUTPUT]:
        ...
        pred_str = []
        if validation:
            logits, loss, loss_numel = self.forward_logits_loss(images, labels, tgt_mask)
        else:
        ...
        return dict(output=BatchResult(total, correct, ned, confidence, label_length, loss, loss_numel, pred_str))
...
class CrossEntropySystem(BaseSystem):
    ...
    def forward_logits_loss(self, images: Tensor, labels: List[str], tgt_mask = None) -> Tuple[Tensor, Tensor, int]:
        targets = self.tokenizer.encode(labels, self.device)
        targets = targets[:, 1:]  # Discard <bos>
        max_len = targets.shape[1] - 1  # exclude <eos> from count
        result = self.forward(images, max_len)
        loss = 0
        
        loss += F.cross_entropy(result[0].flatten(end_dim=1), targets.flatten(), ignore_index=self.pad_id)

        if result[1] is not None and tgt_mask is not None:
            loss += F.binary_cross_entropy_with_logits(result[1].flatten(), tgt_mask.flatten())
        loss_numel = (targets != self.pad_id).sum()  #有多少个有限字符
        return result[0], loss, loss_numel
...
class CTCSystem(BaseSystem):
    ...
    def forward_logits_loss(self, images: Tensor, labels: List[str], tgt_mask = None) -> Tuple[Tensor, Tensor, int]:
        targets = self.tokenizer.encode(labels, self.device)
        logits, mask = self.forward(images)
        ...
        if mask is not None and tgt_mask is not None:
            loss += F.binary_cross_entropy_with_logits(mask.flatten(), tgt_mask.flatten())
