...
class SceneTextDataModule(pl.LightningDataModule):
    ...
    def train_dataset(self):
        if self._train_dataset is None:
            ...
            if self.train_dir not in ("synth", "real"):
                root = PurePath(self.root_dir, 'train', self.train_dir, "Union14M-L-lmdb", "train")
            else:
                root = PurePath(self.root_dir, 'train', self.train_dir)
            '''
            if self.train_dir == "real":
                root = PurePath(self.root_dir, 'train', self.train_dir)
            else:
                root = "../data/train/synth/"
            '''
            self._train_dataset = build_tree_dataset(root, self.charset_train, self.max_label_length,
                                                     self.min_image_dim, self.remove_whitespace, self.normalize_unicode,mask=self.mask,
                                                     transform=transform, train=True, img_size=self.img_size)
    ...
    def val_dataset(self):
        if self._val_dataset is None:
            ...
            if self.train_dir not in ("synth", "real"):
                root = PurePath(self.root_dir, 'train', self.train_dir, "Union14M-L-lmdb", "val")
            else:
                root = PurePath(self.root_dir, 'val')
            self._val_dataset = build_tree_dataset(root, self.charset_test, self.max_label_length,
                                                   self.min_image_dim, self.remove_whitespace, self.normalize_unicode,mask=self.mask,
                                                   transform=transform, train=False, img_size=self.img_size)
    ...
    def test_dataloaders(self, subset):
        ...
        datasets = {s: LmdbDataset(os.path.join(root, s), self.charset_test, self.max_label_length,
                                   self.min_image_dim, self.remove_whitespace, self.normalize_unicode,mask=self.mask,
                                   transform=transform, train=False, img_size=self.img_size) for s in subset}
