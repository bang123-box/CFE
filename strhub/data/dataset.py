...
def lmdb_unsupervised_tree_dataset(roots, *args, **kwargs):
    ...
    for root in roots:
        ...
        for mdb in glob.glob(str(root / '**/data.mdb'), recursive=True):
            ...
            ds_name = str(mdb.parent.relative_to(root))
            ds_root = str(mdb.parent.absolute())
            dataset = UnSupervised_LmdbDataset(ds_root, *args, **kwargs)
            log.info(f'\tlmdb:\t{ds_name}\tnum samples: {len(dataset)}')
            datasets.append(dataset)
    return ConcatDataset(datasets)


def build_json_train_dataset(roots,data_path, *args, **kwargs):
    if not isinstance(roots, list):
        roots = [roots]
    datasets = []
    for root in roots:
        root = Path(root).absolute()
        log.info(f'dataset root:\t{root}')
        for mdb in glob.glob(str(root / '**.json'), recursive=True):
            mdb = Path(mdb)
            ds_name = str(mdb).split("\\")[-1]
            dataset = JSonDataset(data_path, mdb, *args, **kwargs)
            log.info(f'\tlmdb:\t{ds_name}\tnum samples: {len(dataset)}')
            datasets.append(dataset)
    return ConcatDataset(datasets)
...
class LmdbDataset(Dataset):
    ...
    def __init__(self, root: str,  charset: str, max_label_len: int, min_image_dim: int = 0,
                 ...
                 unlabelled: bool = False, mask = False, transform: Optional[Callable] = None, train=True, img_size=[32, 128], transpose: bool = True, trans_aug: bool = False, p: float = 0.0):
        self.mask = mask
        self._env = None
        self.root = root
        self.unlabelled = unlabelled
        self.transform = transform if train == True else None
        self.labels = []
        self.train = train
        self.img_w, self.img_h = img_size[1], img_size[0]
        self.to_tensor = T.Compose([T.ToTensor(),T.Normalize(0.5, 0.5)])
    ...
    def resize_multiscales(self, img, borderType=cv2.BORDER_CONSTANT):
        ...
        return img.resize((self.img_w,self.img_h))
    
    def resize(self, img):
        self.resize_multiscales(img, cv2.BORDER_REPLICATE)
    
    def _process_training(self, image):
        if self.transform != None: image = self.transform(image)
        return self.resize_multiscales(image, cv2.BORDER_REPLICATE)
