...
def main(config: DictConfig):
    ...
    if config.pretrained is not None:
        load_state = torch.load(config.pretrained, map_location=model.device)
        model_state = model.state_dict()
        new_state = {}
        for k, v in model_state.items():
            if k in load_state and v.shape == load_state[k].shape:
                new_state[k] = v
    ...
    checkpoint = ModelCheckpoint(dirpath="./output/" + config.model.name,monitor='val_accuracy', mode='max', save_top_k=3, save_last=True,
